...
# 函数: 安装更新 Docker Compose 环境
install_add_docker_compose() {
	# Alpine Linux 使用 apk 包管理器进行安装
	if [ -f "/etc/alpine-release" ]; then
		# 更新包管理器并安装 Docker Compose
		apk update
		...
		# 在这里加入安装 Docker Compose 的命令，比如使用 curl 或其他方式
		 curl -L "https://github.com/docker/compose/releases/download/X.Y.Z/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
...
EOF
	...
	docker_restart
	echo -e "${green}Docker 已开启 v 6 访问${normal}"
}
...
docker_ipv6_off() {
	...
	docker_restart
	echo -e "${green}Docker 已关闭 v 6 访问${normal}"
}
...
ldnmp_v() {
	# nginx
	if docker exec nginx true > /dev/null 2>&1; then
		# 获取 nginx 版本
...
#	echo -n -e "            mysql : ${huang}v$mysql_version${bai}"
	...
	# 获取 php 版本
	if docker exec php true > /dev/null 2>&1; then
...
#	echo -n -e "            php : ${huang}v$php_version${bai}"
	...
	# 获取 redis 版本
	if docker exec redis true > /dev/null 2>&1; then
...
#		"docker exec php74 sed -i "s/dl-cdn.alpinelinux.org/mirrors.aliyun.com/g" /etc/apk/repositories > /dev/null 2>&1"
		...
		# php 安装包管理
#		"curl -sL https://hub.githubusercontent.com/https://github.com/mlocati/docker-php-extension-installer/releases/latest/download/install-php-extensions -o /usr/local/bin/install-php-extensions > /dev/null 2>&1"
...
output_status() {
	output=$(awk 'BEGIN { rx_total = 0; tx_total = 0 }
		...
		END {
			...
			printf("总接收:%*s%.2f %s\n总发送:%*s%.2f %s\n", 10, "", rx_total, rx_units, 10, "", tx_total, tx_units);
		}' /proc/net/dev)
}
...
ldnmp_install_status_one() {
   if docker inspect "php" &>/dev/null; then
    echo -e "${yellow}LDNMP 环境已安装。无法再次安装。可以使用37. 更新 LDNMP 环境${normal}"
    break_end
    leon
   else
    echo
...
#						;;
					...
					31)
						...
						install curl wget sudo socat htop iftop unzip tar tmux ffmpeg btop gdu fzf cmatrix
						;;

					# 全部卸载
					32)
						clear
						remove htop iftop unzip tmux ffmpeg btop gdu fzf cmatrix
						;;
...
#				echo "9. Docker 页面访问 ▶"
				...
				echo "8. 更换 Docker 源"
				echo -e "${cyan}${bold}------------------------------------------------${jiacu}"
			 	echo "11. 开启 Docker-ipv6 访问"
			  	echo "12. 关闭 Docker-ipv6 访问"
			  	echo -e "${cyan}${bold}------------------------------------------------${jiacu}"
				echo "99. 卸载 Docker 环境"
				echo -e "${cyan}${bold}------------------------------------------------${jiacu}"
				echo "0. 返回主菜单"
				echo -e "${cyan}${bold}------------------------------------------------${jiacu}"
				read -p "请输入你的选择: " sub_choice
...
#				echo "8. 安装 typecho 轻量博客网站"
				...
				echo "24. 站点反向代理 - 域名"
				echo "25. 自定义静态站点"
#				echo "26. 安装 Bitwarden 密码管理平台"
#				echo "27. 安装 Halo 博客网站"
				echo -e "${cyan}${bold}------------------------------------------------${jiacu}"
				echo "31. 站点数据管理"
				echo "32. 备份全站数据"
				echo "33. 定时远程备份"
...
#						cd /home && mkdir -p web/html web/mysql web/certs web/conf.d web/redis web/log/nginx
					...
					24)
						...
						ip_address
						add_yuming
						echo -e "域名格式: ${yellow}http://www.google.com${normal}"
...
#							docker exec mysql mysql -u root -p"$dbrootpasswd" -e "SHOW DATABASES;" 2> /dev/null | grep -Ev "Database|information_schema|mysql|performance_schema|sys"
							...
							docker_exec_output=$(docker exec mysql mysql -u root -p"$dbrootpasswd" -e "SHOW DATABASES;" 2> /dev/null)
                            output=$(echo "$docker_exec_output" | grep -Ev "Database|information_schema|mysql|performance_schema|sys")

                            if [ -n "$output" ]; then
                                echo "$output"
                            else
                                echo -e "${grey}暂无数据库${normal}"
                            fi
					...
					34)
						root_use
						echo -e "请确认 ${yellow}home${normal} 目录中已经放置网站备份的 gz 压缩包，按任意键继续……"
					  	read -n 1 -s -r -p ""
					  	echo "开始解压……"
...
#				echo "31. 留言板                             66. 一条龙系统调优"
				...
				case $sub_choice in
					...
					5)
						root_use
						echo -e "${cyan}打开端口中，请稍后，很快就好......${jiacu}"
						iptables_open
						remove iptables-persistent ufw firewalld iptables-services > /dev/null 2>&1
						echo "端口已全部开放"
						;;
					...
					8)
					  	dd_xitong_1() {
							echo -e "重装后初始用户名: ${yellow}root${normal}  初始密码: ${yellow}LeitboGi0ro${normal}  初始端口: ${yellow}22${normal}"
							echo -e "按任意键继续..."
							read -n 1 -s -r -p ""
							install wget
						...
						dd_xitong_2() {
							echo -e "重装后初始用户名: ${yellow}Administrator${normal}  初始密码: ${yellow}Teddysun.com${normal}  初始端口: ${yellow}3389${normal}"
							read -n 1 -s -r -p ""
							install wget
							wget --no-check-certificate -qO InstallNET.sh 'https://raw.githubusercontent.com/leitbogioro/Tools/master/Linux_reinstall/InstallNET.sh' && chmod a+x InstallNET.sh
						}
						...
						dd_xitong_3() {
							echo -e "重装后初始用户名: ${yellow}root${normal}  初始密码: ${yellow}123@@@${normal}  初始端口: ${yellow}22${normal}"
							read -n 1 -s -r -p ""
							install wget
						...
						dd_xitong_4() {
							echo -e "重装后初始用户名: ${yellow}Administrator${normal}  初始密码: ${yellow}123@@@${normal}  初始端口: ${yellow}3389${normal}"
							echo -e "按任意键继续..."
							read -n 1 -s -r -p ""
							install wget
							curl -O https://raw.githubusercontent.com/bin456789/reinstall/main/reinstall.sh
