...
error_handler() {
    ...
    # 1. 防止报错信息被清屏命令刷掉，先强制输出几个空行
    print_echo
    print_echo
    
    # 2. 醒目的红色报错框
    print_echo "\033[41;37m [FATAL ERROR] 脚本发生致命错误 \033[0m"
    print_echo "\033[31m--------------------------------------------------\033[0m"
    print_echo "出错文件: $0"
    print_echo "出错行号: \033[33m${line_number}\033[0m"
    print_echo "失败命令: \033[33m${command}\033[0m"
    print_echo "退出状态: \033[33m${exit_code}\033[0m"
    
    # 尝试打印函数调用栈（如果是在函数内报错，能知道是谁调用的）
    if [ ${#FUNCNAME[@]} -gt 1 ]; then
        print_echo "函数堆栈: ${FUNCNAME[*]}"
    fi
    print_echo "\033[31m--------------------------------------------------\033[0m"

    # shellcheck disable=SC2034
    read -r -p "⚠️  程序已暂停，请截图报错信息后，按回车键退出..." allow_exit
